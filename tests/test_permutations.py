...
from nn_fourbody_potential.transformations._lookupshortest_permutations import _argmin_over
...
class TestLookupShortestPermutation:
    def test_basic(self, less_than_comparator) -> None:
        """
        Check that the exact minimum permutation and the lookup-shortest minimum
        ...
        approx_output = lookupshortest_minimum_permutation(test_input)

        assert exact_output == pytest.approx(approx_output)

    def test_random(self, less_than_comparator) -> None:
        """
        Check that the exact and lookup-shortest results are the same for many
        random collections of six side lengths. Because the generated side lengths
        are floating-point numbers, the shortest and second shortest side lengths
